 ...
 * 
 * The second part is implemented as two MapReduce jobs where the first does the following:
 * 
 *  1. Map: parse input file and output: ((GSE ID, gene ID), (experiment, expression value))
 *  1. Reduce: create one output file named GSEID-geneID with the 
 */
public class GeoGSE2Pcl extends PerFile {
	enum BatchCounters {
		FILES_READ,
		SER_FILES_READ,
		INVALID_INPUT_FILES,
	...
	public int run(String[] cargs) {		
		...
		try {
			...
			job.setJarByClass(GeoGSE2Pcl.class);
